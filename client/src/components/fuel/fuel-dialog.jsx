...
export function FuelDialog({ open, onOpenChange, expenseToEdit, vehicles, onSubmit }) {
  ...
  }, [expenseToEdit, open, vehicles]);

  // Auto calculate total cost when quantity or unitPrice changes
  const handleQuantityOrPriceChange = (field, value) => {
    setFormData((prev) => {
      const qty = field === "quantity" ? Number(value) : Number(prev.quantity);
      const price = field === "unitPrice" ? Number(value) : Number(prev.unitPrice);
  ...
  const handleChange = (e) => {
    ...
    if (name === "quantity" || name === "unitPrice") {
      handleQuantityOrPriceChange(name, value);
    } else {
      setFormData((prev) => ({
        ...prev,
